...
# tags = pd.read_table('tags.dat', sep='::', header=None, names=tag_headers)

rating_headers = ['user_id', 'movie_id', 'rating', 'rating_id']
ratings = pd.read_table('training_ratings_for_kaggle_comp.csv', sep=',', header=None, names=rating_headers)
...
# df = movies.join(ratings, on=['movie_id'], rsuffix='_r', dtype = float)

del df['movie_id_r']


df['rating'].astype(np.float64)
